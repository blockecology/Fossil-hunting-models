---
title: "Climate-envelope models’ cross-validations"
author: "Sebastian Block et al."
output: html_document
---

This is the code we used for the two cross-validations of each climate-envelope model (results in Table 1 of Block et al. 201X). <br>

First we load and process the data from the [FosSahul database](http://portal.aekos.org.au/dataset/216103).
...
<br>
Then, we select a genus and obtain presence and pseudo-absence data.

```{r Genus presences and pseudo-absences, eval=FALSE}
# Load packages 
require(raster)
require(dismo)
...
<br>
Finally, we run the models and evaluate their predictive performanc with two cross-validation approaches: one spatio-temporal (validation with presence data from different points in space and time) and one purely temporal (validation with presence data from one time-slice not used for training). The spatio-temporal validation is a 5-fold cross validation repeated 100 times to test different combinations of data used for training and testing. 


```{r Validations, eval=FALSE}
...
# Calculate the True Skill Statistics for each model
tbauc <- sapply( teb, function(x){ x@auc } )
txauc <- sapply( tex, function(x){ x@auc } )
tgauc <- sapply( teg, function(x){ x@auc } )
